...
<head>
    ...
    <title>ฟอร์มจัดการรายชื่อผู้มีสิทธิ์สอบ</title>
    <style>
        .table>thead>tr>th {
            padding: 0 5px;
        }
                    ...
                    <div class="row">
                        <div class="col-12">
                            <form id="form-add-std-test">
                                <div class="box">
                                    <div class="box-body">
                                        <h6 class="box-title text-info" style="margin: 0;"><i class="ti-arrow-left" style="cursor: pointer;" onclick="window.location.href='manage_std_test'"></i>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<i class="ti-user mr-15"></i> <b>ฟอร์มจัดการรายชื่อผู้มีสิทธิ์สอบ <?php echo $_GET['term'] ?></b></h6>
                                        <hr class="my-15">
                                        <div class="row">
                                            <div class="col-md-5">
    ...
    <script>
        ...
        async function getSTD_checker() {
            ...
            for (const std_checked of status_check_box) {
                ...
                const status_text = std_checked.checked ? "มีสิทธิ์สอบ" : "ไม่มีสิทธิ์สอบ";
                const obj_std = {
                    std_id: std_id,
        ...
        $('#form-add-std-test').submit(async (e) => {
            ...
            $.ajax({
                type: "POST",
                url: "controllers/std_test_controller",
                data: {
                    std_checked: std_obj,
                    insert_std_test: true,
                ...
                success: async function(json) {
                    if (json.status) {
                        alert(json.msg);
                        window.location.href = 'manage_std_test';
                    } else {
                        alert(json.msg);
                    }
                },
