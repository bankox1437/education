<div class="col-md-12">
    <h5 class="text-center text-dark"><b>วิชาบังคับ</b>
        <?php if (!isset($_GET['mode'])) { ?>
            <span class="badge badge-pill badge-success mr-2" style="cursor: pointer;" onclick="addRow('body_table1')" id="badge_body_table1">เพิ่มวิชา</span>
            <input type="checkbox" id="body_table1_no_subject" class="filled-in chk-col-danger" onchange="disabledRow('body_table1')">
            <label for="body_table1_no_subject">ไม่มีวิชา</label>
        <?php } ?>
    </h5>

    <?php
    if (isset($_GET['set_id']) || isset($_GET['mode'])) {
        $sql = "SELECT * FROM vg_credit_set_compulsory WHERE set_id = :set_id";
            ...
            <tbody id="body_table1">
                ...
                ?>
                        ...
                        if (count($compulsory_data) == 0) { ?>
                            ...
                            <?php } else {
                            foreach ($compulsory_data as $key => $value) {  ?>
                                <tr>
                                    ...
                                    <td style="display: none;"><input type="hidden" value="<?php echo $value->compulsory_id ?>"></td>
                                    <?php echo '<td class="text-center"><span class="badge badge-pill badge-danger" style="cursor: pointer;" onclick="removeRowUpdate(1,' . $value->compulsory_id . ',this)">ลบ</span></td>' ?>
                                </tr>
                        <?php
                                $index++;
                            }
                    ...
                    foreach ($compulsory_data as $key => $value) { ?>
                        <tr>
                            ...
                            <td><input <?php echo $view_mode ?> type="number" class="form-control input-custom-table text-center" autocomplete="off" placeholder="กรอกชื่อรายวิชา" value="<?php echo $value->credit ?>"></td>
                            <td style="display: none;"><input type="hidden" value="<?php echo $value->compulsory_id ?>"></td>
                        </tr>
                <?php }
                }
