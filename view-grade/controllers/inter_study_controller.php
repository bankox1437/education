...
if (isset($_POST['add_data'])) {
    ...
    if (!empty($sub_id)) {
        $sql = "UPDATE vg_inter_study_subject SET sub_code = :sub_code, sub_name = :sub_name, term = :term WHERE sub_id = :sub_id";
        $DB->Update($sql, ["sub_code" => $sub_code, "sub_name" => $sub_name, "term" => $term, "sub_id" => $sub_id]);
    } else {
        $sql = "INSERT INTO vg_inter_study_subject (sub_code, sub_name, std_class,term, user_create) VALUES (:sub_code, :sub_name,:std_class, :term, :user_create)";
    ...
    if ($sub_id > 0) {
        ...
        foreach ($dataColumnWorksheet as $key => $column) {
            ...
            if (empty($work_id)) {
                $sql = "INSERT INTO vg_inter_study_worksheet (sub_id, work_name, user_create) VALUES (:sub_id, :work_name, :user_create)";
                $work_id = $DB->InsertLastID($sql, ["sub_id" => $sub_id, "work_name" => $column['text'], "user_create" => $_SESSION['user_data']->id]);
            } else {
                $sql = "UPDATE vg_inter_study_worksheet SET work_name = :work_name WHERE work_id = :work_id";
            ...
            foreach ($dataStdList[$key] as $std) {
                ...
                if (!empty($inter_id)) {
                    $sql = "UPDATE vg_inter_study_students SET reason = :reason, score = :score, final_score = :final_score, total_score = :total_score, level = :level, sum_score = :sum_score WHERE inter_id = :inter_id";
                    ...
                    $sql = "INSERT INTO vg_inter_study_students (std_id, reason, score, final_score, total_score, level, sum_score, work_id) VALUES (:std_id, :reason, :score, :final_score, :total_score, :level, :sum_score, :work_id)";
                ...
                if ($stdInsert != 0) {
                    $result = true;
                    $sqlCredit = "SELECT * FROM vg_credit WHERE term_id = :term_id AND std_id = :std_id";
                    $dataCredit = $DB->Query($sqlCredit, ["term_id" => $term, "std_id" => $std['std_id']]);
                    $dataCredit = json_decode($dataCredit);
                    if (count($dataCredit) > 0) {
                        $creditId = $dataCredit[0]->credit_id;
                        $sqlCompulsory = "SELECT * FROM vg_credit_compulsory WHERE credit_id = :credit_id AND sub_id = :sub_id";
                        $dataCompulsory = $DB->Query($sqlCompulsory, ["credit_id" => $creditId, "sub_id" => $sub_code]);
                        $dataCompulsory = json_decode($dataCompulsory);

                        $sqlElectives = "SELECT * FROM vg_credit_electives WHERE credit_id = :credit_id AND sub_id = :sub_id";
                        $dataElectives = $DB->Query($sqlElectives, ["credit_id" => $creditId, "sub_id" => $sub_code]);
                        $dataElectives = json_decode($dataElectives);

                        $sqlFreeElectives = "SELECT * FROM vg_credit_free_electives WHERE credit_id = :credit_id AND sub_id = :sub_id";
                        $dataFreeElectives = $DB->Query($sqlFreeElectives, ["credit_id" => $creditId, "sub_id" => $sub_code]);
                        $dataFreeElectives = json_decode($dataFreeElectives);

                        if (count($dataCompulsory) > 0) {
                            $sql = "UPDATE vg_credit_compulsory SET grade = :grade WHERE credit_id = :credit_id AND sub_id = :sub_id";
                            $DB->Update($sql, ["grade" => $std['level'], "credit_id" => $creditId, "sub_id" => $sub_code]);
                            $result = true;
                        }

                        if (count($dataElectives) > 0) {
                            $sql = "UPDATE vg_credit_electives SET grade = :grade WHERE credit_id = :credit_id AND sub_id = :sub_id";
                            $DB->Update($sql, ["grade" => $std['level'], "credit_id" => $creditId, "sub_id" => $sub_code]);
                            $result = true;
                        }

                        if (count($dataFreeElectives) > 0) {
                            $sql = "UPDATE vg_credit_free_electives SET grade = :grade WHERE credit_id = :credit_id AND sub_id = :sub_id";
                            $DB->Update($sql, ["grade" => $std['level'], "credit_id" => $creditId, "sub_id" => $sub_code]);
                            $result = true;
                        }
                    }
    ...
    if (count($storeDeleteInterId) > 0) {
        for ($i = 0; $i < count($storeDeleteInterId); $i++) {
            $sql = "DELETE FROM vg_inter_study_students WHERE inter_id = :inter_id";
    ...
    if (count($storeDeleteWorkId) > 0) {
        for ($i = 0; $i < count($storeDeleteWorkId); $i++) {
            $sql = "DELETE FROM vg_inter_study_worksheet WHERE work_id = :work_id";
    ...
    if (count($storeDeleteStdId) > 0) {
        for ($i = 0; $i < count($storeDeleteStdId); $i++) {
            $sql = "DELETE FROM vg_inter_study_students WHERE std_id = :std_id";
...
if (isset($_GET['getSumScore'])) {
    ...
    $sql_order = "SELECT count(*) totalnotfilter FROM vg_inter_study_subject ss WHERE ss.user_create = :user_create $std_class";
    ...
    $sql = "SELECT * FROM vg_inter_study_subject ss WHERE ss.user_create = :user_create $std_class ORDER BY ss.term ASC,ss.std_class ASC";
...
if (isset($_POST['deleteSumScrore'])) {
    ...
    $sql = "DELETE FROM vg_inter_study_subject WHERE sub_id = :sub_id";
    ...
    foreach ($result as $key => $work) {
        $sql = "DELETE FROM vg_inter_study_students WHERE work_id = :work_id";
        $result = $DB->Delete($sql, ["work_id" => $work->work_id]);
    }

    $sql = "DELETE FROM vg_inter_study_worksheet WHERE sub_id = :sub_id";
    $result = $DB->Delete($sql, ["sub_id" => $sub_id]);
