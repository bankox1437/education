...
class STD_Finish_Model
{
    ...
    function deleteStdGradiate($finish_id)
    {
        $sql = "DELETE FROM vg_std_finish WHERE finish_id = :finish_id";
        $data = $this->db->Delete($sql, ["finish_id" => $finish_id]);
        return $data;
    }
    ...
    function insertStdGradiate($arr_data, $status = "insert")
    {
        ...
        if ($status == "update") {
            ...
            $where = "WHERE finish_id = :finish_id";
        }
        $sql = $action . " vg_std_finish SET " . $column . $where;
    ...
    function getDataStdFinish()
    {
        ...
        $term_name = " finish.term_id = " . $_SESSION['term_active']->term_id . " \n";
        if (isset($_REQUEST['term_id']) && $_REQUEST['term_id'] != "0") {
            if (empty($std_class)) {
                $term_name = " finish.term_id = " . $_REQUEST['term_id'] . " \n";
            } else {
                $term_name = " AND finish.term_id = " . $_REQUEST['term_id'] . " \n";
            }
        } else  if (isset($_REQUEST['term_id']) && $_REQUEST['term_id'] == "0") {
            $term_name = " ";
        }

        $user_condition = " finish.user_create = " . $_SESSION['user_data']->id . "\n";
        $address = "";
        $admin_join = "";
        $where_address = "";
        if ($_SESSION['user_data']->role_id == 1) {
            ...
            $admin_join = "LEFT JOIN tb_users u ON finish.user_create = u.id\n" .
        ...
        //นับจำนวนทั้งหมด
        $sql_order = "SELECT count(*) totalnotfilter FROM vg_std_finish finish\n" .
            $whereTotal;
        ...
        $sql = "SELECT\n" .
            ...
            "	std.std_code,finish.std_class \n" .
            $address .
            "FROM\n" .
            "	vg_std_finish finish\n" .
            "	LEFT JOIN vg_terms term ON finish.term_id = term.term_id\n" .
            "	LEFT JOIN tb_students std ON finish.std_id = std.std_id\n" .
            $admin_join .
    ...
    function stdGetDataStdFinish()
    {
        ...
        $sql = "SELECT\n" .
            ...
            "	LEFT JOIN tbl_non_education edu ON users.edu_id = edu.id \n" .
            "WHERE\n" .
            "	edu.id = :edu_id " . $std_class . $term_name;
