...
class STD_Gradiate_Model
{
    ...
    function deleteStdGradiate($gradiate_id)
    {
        $sql = "DELETE FROM vg_gradiate WHERE gradiate_id = :gradiate_id";
        $data = $this->db->Delete($sql, ["gradiate_id" => $gradiate_id]);
        return $data;
    }

    function insertStdGradiate($arr_data, $status = "insert")
    {
        $action = "INSERT INTO ";
        $where = "";
        ...
        if ($status == "update") {
            ...
            $where = "WHERE gradiate_id = :gradiate_id";
        }
        $sql = $action . " vg_gradiate SET " . $column . $where;
    ...
    function getDataStdGradiate()
    {
        ...
        $term_name = " gra.term_id = " . $_SESSION['term_active']->term_id . " \n";
        if (isset($_REQUEST['term_id']) && $_REQUEST['term_id'] != "0") {
            if (empty($std_class)) {
                $term_name = " gra.term_id = " . $_REQUEST['term_id'] . " \n";
            } else {
                $term_name = " AND gra.term_id = " . $_REQUEST['term_id'] . " \n";
            }
        } else  if (isset($_REQUEST['term_id']) && $_REQUEST['term_id'] == "0") {
            $term_name = " ";
        }

        $user_condition = " gra.user_create = " . $_SESSION['user_data']->id . "\n";
        $address = "";
        $admin_join = "";
        $where_address = "";
        if ($_SESSION['user_data']->role_id == 1) {
            ...
            $admin_join = "LEFT JOIN tb_users u ON gra.user_create = u.id\n" .
        ...
        //นับจำนวนทั้งหมด
        $sql_order = "SELECT count(*) totalnotfilter FROM vg_gradiate gra\n" .
            $whereTotal;
        ...
        $sql = "SELECT\n" .
            ...
            "	std.std_code,gra.std_class \n" .
            $address .
            "FROM\n" .
            "	vg_gradiate gra\n" .
            "	LEFT JOIN vg_terms term ON gra.term_id = term.term_id\n" .
            "	LEFT JOIN tb_students std ON gra.std_id = std.std_id\n" .
            $admin_join .
            "WHERE\n" . $std_class . $term_name . $user_condition_and;
        $sql_total = $sql . $search . $where_address . " \n ORDER BY $order ";
    ...
    function stdGetDataStdGradiate()
    {
        $std_class = "";
        if (isset($_REQUEST['std_class']) && !empty($_REQUEST['std_class'])) {
            $std_class = " AND gra.std_class = '" . $_REQUEST['std_class'] . "' ";
        }
        ...
        $sql = "SELECT\n" .
            ...
            "	LEFT JOIN tbl_non_education edu ON users.edu_id = edu.id \n" .
            "WHERE\n" .
            "	gra.std_id = :std_id " . $std_class . $term_name;
