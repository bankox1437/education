...
function formatButtonEdit(data, row) {
    let html = `<a href="manage_credit_new_edit?credit_id=${row.credit_id}">
...
function formatButtonDelete(data, row) {
    let html = `<button type="button" onclick="deleteCredit(${row.std_id})" class="waves-effect waves-circle btn btn-circle btn-danger mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-trash" style="font-size:10px"></i></button>`;
    return html;
}

function formatButtonView(data, row) {
    let html = `<a href="manage_credit_new_view?mode=view&std_id=${row.std_id}">
