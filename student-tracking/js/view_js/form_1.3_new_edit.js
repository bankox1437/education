...
function getDataVisit() {
  ...
  $.ajax({
    ...
    data: {
      getDataStudentPerEdit: true,
      form_visit_id: params.form_visit_id,
    },
    success: function (data) {
      const jsondata = JSON.parse(data);
      console.log(jsondata);
      genHtmlTable(jsondata.msg);
    },
  });
}
              ...
              <td onclick="checkedbehavior('${element.title_id}_${element.sub_title_id}')">
                <div class="form-group text-center">
                      <div class="c-inputs-stacked">
                          <input name="${element.title_id}" type="radio" id="${element.title_id}_${element.sub_title_id}" data-det-id="${element.form_v_sum_det_id}" class="with-gap radio-col-info input-vid" value="${element.sub_title_id}" ${check_radio}>
...
function saveData() {
  ...
  for (var i = 0; i < radio_checked.length; i++) {
    ...
    if (radio_checked[i].checked) {
      object = {
        v_sum_det_id: radio_checked[i].getAttribute('data-det-id'),
