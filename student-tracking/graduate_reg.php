...
<head>
    ...
    <title>ทะเบียนผู้จบการศึกษา</title>
    <style>
        #table td {
            padding: 1px 5px;
        }
...
<body class="hold-transition light-skin sidebar-mini theme-primary">
                            ...
                            <div class="box">
                                <div class="box-header with-border">
                                    <div class="row align-items-center">
                                        <h4 class="box-title">ทะเบียนผู้จบการศึกษา</h4>
                                        <?php if ($_SESSION['user_data']->role_id == 3) { ?>
                                            <a class="col-md-2 waves-effect waves-light btn btn-success btn-flat ml-2 my-2" href="graduate_reg_add"><i class="ti-plus"></i>&nbsp;บันทึกข้อมูลผู้จบการศึกษา</a>
                                            <div class="form-group ml-2 mb-0 col-md-4">
                                                <div class="input-group">
                                                    <input value="<?php echo $searchSession ?>" type="text" id="search" class="form-control" placeholder="ค้นหาด้วยชื่อ-สกุล หรือ เลขประจำตัวประชาชน" autocomplete="off" onkeyup="isEmptyInputSearch();search()">
                                        ...
                                        <?php if ($_SESSION['user_data']->role_id != 3) { ?>
                                            <div class="col-md-4">
                                                <div class="form-group ml-1 mb-0">
                                                    <div class="input-group">
                                                        <input value="<?php echo $searchSession ?>" type="text" id="search" class="form-control" placeholder="ค้นหาด้วยชื่อ-สกุล หรือ เลขประจำตัวประชาชน" autocomplete="off" onkeyup="isEmptyInputSearch()">
                                                        <div class="input-group-append" onclick="search()">
                                                            <span class="input-group-text"><i class="ti-search button-icon-search"></i></span>
                                                        </div>
                                                    </div>
                                ...
                                <div class="box-body no-padding">
                                    <table id="table" data-icons="icons" data-show-refresh="true" data-minimum-count-columns="2" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, all]" data-side-pagination="server" data-url="controllers/gradiate_reg_controller?getDataGraReg=true">
                                    </table>
                                </div>
                                <!-- /.box-body -->
                            </div>
    ...
    <script>
        ...
        function formatButtonView(data, row) {
            let html = `<a href="uploads/gradiate_reg/${row.file_name}" target="_blank">
        ...
        function formatButtonEdit(data, row) {
            let html = `<a href="graduate_reg_edit?gra_reg_id=${row.gra_reg_id}">
        ...
        function formatButtonDelete(data, row) {
            let html = `<button type="button" onclick="deleteGraReg(${row.gra_reg_id},'${row.file_name}')" class="waves-effect waves-circle btn btn-circle btn-danger mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-trash" style="font-size:10px"></i></button>`;
            return html;
        }
            ...
            $table.bootstrapTable("destroy").bootstrapTable({
                ...
                columns: [{
                    ...
                    },
                    {
                        ...
                        title: "ผู้บันทึก",
                        width: "180px",
                        visible: role_id == 1 || role_id == 2 ? true : false
                    },
    ...
    <script>
        ...
        function getBywhere() {
            ...
            var urlWithParams = $table.data('url') + `&search=${searchparam}&pro_id=${pro_id}&dis_id=${dis_id}&sub_dis_id=${sub_dis_id}` +
                '&term_id=' + term_id;
        ...
        function deleteGraReg(gra_reg_id, file_name) {
            const confirmDelete = confirm('ต้องการลบผู้จบการศึกษานี้หรือไม่?');
            if (confirmDelete) {
                $.ajax({
                    type: "POST",
