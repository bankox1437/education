...
if (isset($_POST['getDataEducationEdit'])) {
    $response = array();
    try {
        $eduModel = new Education_Model($DB);
    ...
    // print_r($array_data);
        $edu_data = $eduModel->getEducationEdit($array_data);
        $response = array('status' => true, 'data' => $edu_data);
        echo json_encode($response);
    } catch (Exception $e) {
        $response = array('status' => false, 'data' => [], 'msg' => $e->getMessage());
        echo json_encode($response);
...
if (isset($_POST['getDataEducationOtherEdit'])) {
    $response = "";
    try {

        $eduModel = new Education_Model($DB);
        $result = $eduModel->getEducationOther();
