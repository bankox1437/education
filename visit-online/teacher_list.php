...
$guideRole = isset($status->guide) ? true : false;
...
<head>
    ...
    <title>นิเทศออนไลน์-รายชื่อครูตำบล</title>
</head>
...
<body class="hold-transition light-skin sidebar-mini theme-primary">
                        ...
                        <div class="col-12">
                            <div class="box">
                                ...
                                <div class="box-body no-padding">
                                    <table id="table" data-icons="icons" data-minimum-count-columns="2" data-pagination="true" data-id-field="id" data-page-list="[10, 25, 50, 100, all]" data-side-pagination="server" data-url="controllers/am_controller?getDataUsersBT=true">
                                    </table>
                                </div>
                                <!-- /.box-body -->
                            </div>
    ...
    <script>
        let calendar_new = '<?php echo $calendar_new ?>';
        let guideRole = '<?php echo $guideRole ?>';
        var $table = $("#table");
        var $remove = $("#remove");
        var selections = [];
        ...
        function checkEduFormat(data, row) {
            if (row.role_custom_id) {
                return row.role_cus_name;
            }
            if (row.edu_name == null) {
                return `-`;
            }
            return row.edu_name + ` (แอดมินครู)`;
        }
        ...
        function checkDistrictFormat(data, row) {
            ...
            if ((row.district == null && row.role_id == 2) || row.role_custom_id) {
        ...
        function checkProvinceFormat(data, row) {
            ...
            if ((row.province == null && row.role_id == 2) || row.role_custom_id) {
        ...
        function opsView(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_calendar?pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-success mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-eye"></i></button>
                    </a>`
            } else {
                return "-"
            }
        ...
        function opsView1(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_calendar?class=1&pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-success mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-eye"></i></button>
                    </a>`
            } else {
                return "-"
        ...
        function opsView2(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_calendar?class=2&pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-success mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-eye"></i></button>
                    </a>`
            } else {
                return "-"
        ...
        function opsView3(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_calendar?class=3&pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-success mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-eye"></i></button>
                    </a>`
            } else {
                return "-"
        ...
        function opsView4(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_sum_score?pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-warning mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-bar-chart-alt"></i></button>
                    </a>`
            } else {
                return "-"
            }
        ...
        function opsReport(data, row) {
            ...
            return ` <a href="manage_summary?pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-primary mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-agenda"></i></button>
                    </a>`
        ...
        function opsSummary(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="manage_index?pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-info mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-zip"></i></button>
                    </a>`
            } else {
                return "-"
            }
        ...
        function opsGuide(data, row) {
            if (!row.role_custom_id) {
                ...
                return ` <a href="../view-grade/manage_guide_std?pro=${pro}&dis=${dis}&sub=${sub}&page_number=${currentPageNumber}&user_id=${row.id}&name=${row.name} ${row.surname}">
                        <button type="button" class="waves-effect waves-circle btn btn-circle btn-warning mb-5 mt-1" style="width:30px;height:30px;"><i class="ti-zip"></i></button>
                    </a>`
            } else {
                return "-"
            }
                ...
                columns: [
                    [{
                        ...
                        },
                        {
                            title: "ช่องทางการสอน",
                            align: "left",
                            formatter: checkChannelFormat
                        },
                        {
                            title: "สถานศึกษา/ตำแหน่ง",
                            align: "left",
                        ...
                        },
                        {
                            ...
                            align: "center",
                            formatter: opsView,
                            visible: (role_id == 1 || parseInt(calendar_new) == 1) ? false : true
                        },
                        {
                            title: "นิเทศ ประถม",
                            align: "center",
                            formatter: opsView1,
                            visible: (role_id == 1 || parseInt(calendar_new) == 1) ? true : false
                        },
                        {
                            title: "นิเทศ ม.ต้น",
                            align: "center",
                        ...
                        },
                        {
                            title: "ประเมิน",
                            align: "center",
                            formatter: opsSummary
                        },
                        {
                            title: "ห้องแนะแนว",
                            align: "center",
                            formatter: opsGuide,
                            visible: parseInt(guideRole) == 1 ? true : false
                        },
                    ],
                ],
            });
